...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const initialCartItems: CartItem[] = cartItemsFromStorage ? JSON.parse(cartItemsFromStorage) : [];
  ...
  const changeQuantity = (item: CartItem, amount: number) => {
    ...
    if (isItemInCart) {
      ...
      if (updatedQuantity >= 1 && updatedQuantity <= 99) {
        setCartItems(
          cartItems.map((cartItem) =>
            cartItem.product.id === item.product.id ? { ...cartItem, quantity: updatedQuantity } : cartItem
          )
        );
      }
    }
