...
export const CartPage = () => {
  const { cartItems, removeFromCart, changeQuantity, getCartTotalPrice } = useCart()
  return (
            ...
            <div className="bg-card w-full rounded-[30px] p-[18px] shadow flex gap-[24px]">
              <div className="max-w-[150px]">
                ...
                <div className="flex gap-x-[22px] justify-center mt-[20px] items-center">
                  <button className="size-[30px] bg-[#FFCE7F] rounded-full" onClick={() => changeQuantity(item, -1)}>-</button>
                  <p className="font-semibold text-[17px]">{item.quantity}</p>
                  <button onClick={() => changeQuantity(item, 1)} className="size-[30px] bg-[#FFCE7F] rounded-full">+</button>
                </div>

              </div>
              <div className="self-center space-y-[12px] flex-1">
                <p className="text-[#1C1C27] text-[17px] font-medium">{item.product.title}</p>
                <p className="text-[#AAAAAA] text-[15px] font-semibold">{item.product.price} ₽</p>
              </div>
              <div className="flex flex-col justify-between items-end">
                <button onClick={() => removeFromCart(item)}>Удалить</button>
                <p className="text-[#1C1C27] text-[15px] font-semibold">{item.product.price} ₽</p>
              </div>
